...
def get_action_from_model(model_output: dict[str, np.ndarray], prev_action: log.ModelDataV2.Action, v_ego: float,
                          long_action_t: float, lat_smooth_seconds: float, vEgoStopping: float) -> log.ModelDataV2.Action:
    desired_accel = model_output['action'][0,1]
    desired_curvature = model_output['action'][0,0] / (max(1.0, v_ego))**2
    should_stop = (v_ego < 0.3 and desired_accel < 0.1)

    plan = model_output['plan'][0]
    _, should_stop, _, desired_velocity_now = get_accel_from_plan(plan[:,Plan.VELOCITY][:,0],
...
def main(demo=False):
  ...
  while True:
    ...
    if model_output is not None:
      ...
      action = get_action_from_model(model_output, prev_action, v_ego, long_action_t, lat_smooth_seconds_dynamic, vEgoStopping)
      prev_action = action
